...
#if defined(_WIN32)
static int randombytes_win32_randombytes(unsigned char* buf, long long n)
{
	HCRYPTPROV ctx;
	BOOL tmp;
...
static int randombytes_linux_randombytes_getrandom(unsigned char *buf, long long n)
{
...
static int randombytes_linux_randombytes_urandom(unsigned char *buf, long long n)
{
	int fd;
...
#if defined(BSD)
static int randombytes_bsd_randombytes(unsigned char *buf, long long n)
{
...
#if defined(__EMSCRIPTEN__)
static int randombytes_js_randombytes_nodejs(unsigned char *buf, long long n) {
	int ret = EM_ASM_INT({
...
# pragma message("Using crypto api from NodeJS")
	randombytes_js_randombytes_nodejs(buf, n);
...
# pragma message("Using Windows cryptographic API")
	/* Use windows API */
	randombytes_win32_randombytes(buf, n);
#else
